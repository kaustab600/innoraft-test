...
// -------------searching for highest scorer in playersrun array---------------------------//
  ...
  echo $player_with_highestScr." ".$max."<br>";
...
$per_match_winner = [];// it stores winners of each match

$tournamentwinner = [];// this array stores the number of wins by each team

foreach($tournament as $match => $team)
  {
    foreach($team as $teamstat => $value)//$teamstat is [teamstat] object
    {
      foreach($value as $team => $v1)
      {  
        ...
        foreach($v1 as $player => $runs)
          {
              //A=140
            $total = $total + $runs;
            $maxscore[$j][$team] = $total;// to store the runs made by each team per match 
              //B=100
            if($max<$total)
            {
